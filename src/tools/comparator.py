...
class InvoiceComparator:
        ...
        return DataFrame(
            {
                "invoice_number": concat(
                    [self.df_pdf["invoice_number"], self.df_xlsx["invoice_number"]]
                ).unique()
            }
        )
    ...
    def _merge_data_frames(self) -> DataFrame:
        ...
        data_merged = invoice_ids.merge(self.df_xlsx, how="left", on="invoice_number")
        data_merged = data_merged.merge(self.df_pdf, how="left", on="invoice_number")
        data_merged.rename(
